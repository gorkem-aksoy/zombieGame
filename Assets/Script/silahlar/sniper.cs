...
public class sniper : MonoBehaviour
{
    ...
    public float menzil;
    public Camera benimCam; // raycast ýþýný için
    ...
    void Update()
    {
        ...
        */
        ...
        if (Input.GetKey(KeyCode.Mouse0) && !genelKontrol.oyunDurdumu)
        {
        ...
        {
            // zoom yapýlacaðý zaman true parametre gönder
            kameraYakinlasScopeCikar(true);

        }
        else if (Input.GetKeyUp(KeyCode.Mouse1))
        {
            // zoom da çýkýlacaðý zaman false parametre gönder
            kameraYakinlasScopeCikar(false);
        }
    ...
    // þarjor deðiþtirme sýrasýnda animasyona event verilerek eriþildi
    void bosKovanCikis()
    {
        //silahtan boþ kovan çýkýþý
        // boþ kovana box colider verdiðinde yürürken ileri gidiþi engelliyor ?
        //karakter ileri giderse geri doðru uçuyor, geri giderse ileri doðru uçuyor, belirli bir yön verme ?
        //normal
        //Instantiate(kovanObjesi, kovanCikisNoktasi.transform.position, kovanObjesi.transform.rotation);

        // kovan çýkarma ve silah bozulma yapýlabilir. Ateþ ettiðinde kovan çýkmýyorsa, sýkýþmýþtýr.
        if (kovanCiksinmi)
        {
            GameObject bosKovan = Instantiate(kovanObjesi, kovanCikisNoktasi.position, kovanCikisNoktasi.rotation);
            Rigidbody rg = bosKovan.GetComponent<Rigidbody>();
            rg.AddRelativeForce(new Vector3(-10f, 1, 0) * 45f);
        }
        else
        {
            Debug.Log("silah bozuldu");
        }
        //rg.AddForce(kovanCikisNoktasi.transform.right * -60f, ForceMode.Impulse);
        //rg.velocity = kovanCikisNoktasi.transform.right * 10f;
    ...
    void mermiEkle(string silahTuru, int mermiSayisi)
    {
        ...
        switch (silahTuru)
        {
            ...
            case "magnum":
                ...
                gelenMevcutMermi = PlayerPrefs.GetInt("magnum_mermi"); // pompali_mermi key'inden mermi sayýsýný alýyoruz, eðer key yoksa 0 döner
                gelenMevcutMermi += mermiSayisi; // Yeni mermiyi ekliyoruz
                PlayerPrefs.SetInt("magnum_mermi", gelenMevcutMermi); // Güncellenen mermi sayýsýný kaydediyoruz
    ...
    // mouse sað týk'a basýldýðýnda çaðýrýlacak.
    void kameraYakinlasScopeCikar(bool durum)
    {
        if (durum)
        {
            // gelen parametre true ise
            cross.SetActive(false);
            
            benimAnimator.SetBool("zoomYap", durum);

            // camera da culling mask'e eriþ 8. katmaný gizle
        ...
        {
            // gelen parametre false ise
            cross.SetActive(true);

            benimAnimator.SetBool("zoomYap", durum);

            // tüm katmanlarý göster demektir.
