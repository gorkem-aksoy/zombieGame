...
public class pompali : MonoBehaviour
{
    ...
    public Transform[] mermiCikisNoktalari;
    ...
    void atesEt(bool yakinlasmaVarmi)
    {
        ...
        StartCoroutine(cameraTitret(.15f, .5f));

        for (int i = 0; i < mermiCikisNoktalari.Length; i++)
        {
            Instantiate(mermiObjesi, mermiCikisNoktalari[i].transform.position, mermiCikisNoktalari[i].transform.rotation);
        }
    ...
    // kalan mermi ve toplam mermi sayýsýný set etme iþlemleri
    void mermiSetEt()
    {
    ...
    // zoom animasyonunda ilgili frame de çaðýrýlýyor.
    ...
    // atesEt Fonksiyonunda çaðýrýlabilir.
    IEnumerator cameraTitret(float titremeSuresi, float magnitude)
    {
